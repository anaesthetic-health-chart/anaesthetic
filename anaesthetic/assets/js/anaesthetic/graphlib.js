...
NHGraph = (function(superClass) {
  ...
  NHGraph.prototype.draw = function(parent_obj) {
    var background_object, index, j, k, key, len, len1, ref, ref1, results, self;
    self = this;
  ...
  NHGraph.prototype.draw_ranged = function(obj, key_index) {
    var keys;
    if (key_index == null) {
      key_index = false;
    }
    ...
    if (keys.length === 2) {
      ...
      throw new Error('Cannot plot ranged graph with ' + keys.length + ' data point(s)');
    }
  };
  ...
  NHGraph.prototype.draw_linear = function(obj, key_index, style) {
    ...
    obj.drawables.data.selectAll(".point" + key_index).data(obj.parent_obj.parent_obj.data.raw.filter(function(d) {
      ...
      return obj.show_popup(d[obj.options.keys[key_index]], event.pageX, event.pageY);
    }).on('mouseout', function(d) {
      return obj.hide_popup();
    });
  ...
  NHGraph.prototype.redraw = function(parent_obj) {
    var adjusted_line, index, j, key, len, ref, self, tick_font_size, tick_line_height;
    self = this;
  ...
  NHGraph.prototype.redraw_linear = function(obj, key_index, style) {
    if (key_index == null) {
      key_index = 0;
    }
    if (style == null) {
  ...
  NHGraph.prototype.redraw_ranged = function(obj, key_index) {
    var keys;
    if (key_index == null) {
      key_index = false;
    }
    keys = key_index ? obj.options.keys[key_index] : obj.options.keys;
    obj.drawables.data.selectAll(".range.top").data(obj.parent_obj.parent_obj.data.raw).enter().append("rect").attr({
      'y': function(d) {
        return obj.axes.y.scale(d[[0]]);
      },
      'x': function(d) {
    ...
    obj.drawables.data.selectAll(".range.bottom").data(obj.parent_obj.parent_obj.data.raw).enter().append("rect").attr({
      'y': function(d) {
        return obj.axes.y.scale(d[keys[1]]);
      },
      'x': function(d) {
    ...
    return obj.drawables.data.selectAll(".range.extent").data(obj.parent_obj.parent_obj.data.raw).enter().append("rect").attr({
      'y': function(d) {
        return obj.axes.y.scale(d[keys[0]]);
      },
      'x': function(d) {
